...
function draw(canvas, toCanvas, vertices, camera) {
  function drawVertices(v, cameraPlane) {
    v.forEach(vertex => {
      // Project the vertices to the camera plane
      // and then convert the coordinates to canvas
      const x1 = toCanvas(project(vertex[0], cameraPlane));
      const x2 = toCanvas(project(vertex[1], cameraPlane));
      ctx.beginPath();
  ...
  const cameraPlane = normalizeCameraPlane(camera);
  // Draw axis
  ...
  drawVertices(vertices, cameraPlane);
}
